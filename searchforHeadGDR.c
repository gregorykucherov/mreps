...
extern int *factorEnds, numFact, numGlobFact, numFullBlock;
...
extern int lenWrd, minPer, maxPer, actPerBnd;
extern listreps *presortLists;


void mainstage(void)
{  subRepBgn=subRepEnd=(int *)calloc(maxNumErr+1, sizeof(int));
   if( ( presortLists=(listreps *) calloc ((2+lenWrd), sizeof(listreps)) 
	) == NULL )
...
void FindFinalGlobalHeadGDR(void)
{  curHeadPstn=lenWrd+1;
   curMaxPer=maxSumFact[curBlock]-2;
   Period=minPer; 
   if (lenWrd>2*curMaxPer)
   {  if (curMaxPer<maxPer)
      {  while(Period<=curMaxPer)
         {  FindFinalHeadGDR0();
            Period++;  }
